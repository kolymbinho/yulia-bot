...
await update.message.reply_text(
    ...
    "💡 Напиши идею: кто он, какой у него характер, как общается, говорит ли от первого лица и т.д.\n\n"
    "💵 Стоимость кастомного персонажа — *150 грн*\n"
    "💳 Карта для оплаты: *5375 4141 2345 6789*\n"
    "📩 После оплаты — просто пришли сюда свою идею, и я всё сделаю вручную!"
)
return
            ...
            if user_id != ADMIN_ID:
                if user_id not in unlocked_chars or key not in unlocked_chars[user_id]:
                    await update.message.reply_text(
                        ...
                        "💳 Переведи на карту *5375 4141 2345 6789*\n"
                        "📩 Потом просто напиши: *«Разблокировать [имя персонажа]»* — и я всё активирую вручную!"
                    )
                    return  # ВОТ ЭТО ВАЖНО! Только если персонаж платный и не разблокирован
